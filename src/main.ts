...
engine.onFinish = (s) => {
  const best = Math.min(...s.player.laps),
    courseVersion = s.track.id === 'storm' ? 'v11' : s.track.id === 'palms' ? 'v8' : 'v9',
...
const updateCheckpoint = setupCheckpoint($('checkpoint'), engine);
engine.onRender = () => {
  multiplayer.render();
  updateCheckpoint();
};
