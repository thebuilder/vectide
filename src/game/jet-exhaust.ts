...
export class JetExhaust {
  ...
  update(racer: Racer, dt: number, throttle = Math.min(Math.hypot(racer.vx, racer.vz) / 8, 1)) {
    ...
    const boost = racer.boost > 0 ? Math.min(racer.boostPower - 1, 1.8) : 0;
    const driving = racer.recovery.phase === 'riding';
    const target = driving ? Math.max(0, Math.min(1, throttle)) * (0.2 + speed * 0.8) : 0;
    ...
    const width = (0.5 + this.amount * 0.5 + boost * 0.2) * pulse;
    this.group.scale.set(width, width, this.amount * (1.8 + boost * 1.2) * pulse);
    this.shell.material.opacity = 0.28 + this.amount * 0.15 + boost * 0.08;
