...
- `pnpm check`: 346 tests across 51 files pass, including full six-rider three-lap simulations on all courses, pickup races, twelve-rider multiplayer simulation, gate admission/recovery, landing physics, water contact, dolphins and rendering interpolation.
...
## Checkpoint model and fabric follow-up

- Dark faceted masts and short brackets separate the wider pink/mint swallowtail flags from their poles. A painted double-chevron emblem follows the fabric surface.
- The active gate's cloth has a pinned leading edge, gentle sag and continuous traveling ripples with shaded folds. Beat strength never changes the fabric pose. A closed beacon ring and mast light carry the beat pulse; other gates stay still and dimmer.
- Muting music removes beacon pulses. Reduced motion retains a static fabric pose and steady beacon. Shared model dimensions keep the viewport guidance bounds around the wider flags. Owned paint textures are released when switching worlds.
- All 346 tests and the production build pass. New tests cover animated bounds, active gate changes, beat-independent cloth, muted/reduced motion, frozen/hidden cloth caching and texture cleanup. Six native browser cases passed across all three actual soundtracks, a full Storm lap, visible/offscreen/finish gate views and phone rotation. Final music checks measured p95 frame times of 16.7–16.8 ms on this Mac and verified active-only motion, music mute and reduced-motion cloth stability. Actual three-course renders and an eight-second recorded preview were inspected. Independent review found no remaining issues.
