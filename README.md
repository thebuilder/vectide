...
`src/game/water.ts` owns three directional wave components and a four-meter world-space grid. The water vertex shader uses those same components. The CPU interpolates the same mesh triangles rather than sampling a different smooth surface.
