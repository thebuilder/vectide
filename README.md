...
Open the local address printed by Vite. Enable hardware acceleration in your browser. Keyboard, standard gamepad, and multitouch driving are supported. On phones, throttle is automatic. Slide the virtual stick left/right to steer and up/down to lean forward/back. Hold BRAKE to cut throttle and slow down. Hold JUMP to prepare a stunt, then release as a wave or ramp launches you. Pull the stick back/forward to flip, or sideways to spin. Hold to keep rotating; release near upright to settle. RESET appears after a missed gate, when far off course, or after getting stuck for two seconds. The pause icon is at the top right. Its menu has separate Sounds and Music sliders, with remembered levels and a curved volume response for quieter low settings. Music defaults to 60%, with headroom for pickup and item cues; the engine has its own quieter mix. Hit the Water opens course, mode and difficulty selection in the same screen. Back returns to the title while retaining selections. Compact screens scroll course options while keeping Start Race visible. Controls are available from pause; help opens at its heading with current-device basics first. First-race tips advance after successful acceleration, gate steering and braking, and remember progress separately for keyboard, gamepad and touch. Optional jump guidance waits until a nearby ramp.
...
| E                                  | Hold to prepare; release at takeoff, then lean or steer |
| Gamepad RB                         | Hold to prepare; release at takeoff, then use the stick |
| JUMP (touch)                       | Hold to prepare; release at takeoff, then use the stick |
...
Palm and Storm each have five named checkpoints, including the finish. Port has seven, with inner-basin and southwest-basin gates that prevent the broad diagonal shortcuts. They guard route choices instead of dividing every straight into equal sections. Islands, reefs and breakwaters block the main cuts, while gate angles follow the water route, with Port’s basin gates facing the incoming leg. Ranking and AI recovery account for the longer sections. Storm's start line faces its first checkpoint across an open launch straight. Palm sweeps toward a reef-entry gate before meeting the main wave train, then settles on calm water before passing the exit gate. Its nearby headlands leave room to line up for the waves. It has a pair of linked optional ramps plus a smaller opening ramp, and turns directly toward the finish after the jump straight. Port has no ramps: its dock passages and sheltered basins lead into a direct final approach. Storm runs counterclockwise, with a west wave train, an open-water weave, and broad sweeps through the east-channel swell instead of ramps. Its western reef and shore leave open checkpoint approaches, the weave checkpoint sits before the turn, and the signal platform sits farther offshore. Each buoy follows its own local wave height. Eight-meter masts, bright pink/cyan pennants and larger overhead arrows keep the next opening readable above the swell. Palm's early bends have wider gates and shore clearance for weapon knockback; the eastern channel puts a sustained wave train across the racing line. Normal AI laps are about one minute; a three-lap race takes around three minutes. Race traffic and waves change lap times. Easy, Normal, and Expert change opponent pace; player handling stays consistent. Normal uses full throttle on open water and firmer braking in tight turns, while Expert carries more pace on calm courses and manages rough-water turns more conservatively. Flat-water cruising speed is approximately 82 km/h.
...
Load a trick by holding E or gamepad RB on the approach, then release near takeoff. After releasing, Shift/C or vertical stick movement controls flip rotation; A/D or horizontal stick movement controls spin rotation. Holds build angular velocity and can continue through multiple turns. Releasing direction damps momentum, with a small assist within 25 degrees of alignment; the opposite direction brakes and then reverses rotation. A neutral prepared jump has no automatic trick. W stays throttle in the air. Releases up to 300 ms before takeoff are buffered; earlier releases cancel. Holding visibly crouches the rider. Landing is judged by hull attitude relative to the water or ramp and the impact speed, rather than a stunt completion cutoff. An imperfect upright flip loses some speed; an unfinished spin keeps its actual heading and world momentum, so it skids into the new direction. The rotated hull supplies the contact samples during a stunt. Inverted or steep, hard entries throw the rider off; the rider falls, swims back beside the craft, and climbs aboard. Landing a completed flip adds 10 percent to horizontal speed, capped at 2 m/s, once per landing. Checkpoints cannot advance while detached. Ordinary jumps remain forgiving. A forward-lean entry briefly pierces the water with less impact damping; drag slows the submerged craft and full buoyancy returns it to the surface. Flat-water leaning cannot sustain a dive.

The close chase camera follows horizontal movement directly, maintaining its distance at speed, and stays above the wave surface. Local races interpolate craft position, orientation and rider load between the two latest 120 Hz physics steps. The camera, water and contact effects use that same presentation time, so uneven display timing does not repeat a pose and then jump ahead. Pausing holds the presentation state and resets discard its history. The game pauses when focus is lost or the page is hidden. The course menu shows a single floating craft with course markers hidden. Multiplayer lobbies show the joined racers lined up on the water. Palm islands use low, irregular faceted ground, narrow sand beaches and small rock outcrops. Slender tapered trunks support folded, serrated palm crowns, with roots placed on the actual terrain. A shared shoreline distance field tapers the rendered waves and buoyancy near land; small breakers travel toward the coast and dissolve into broken foam. Offshore swell remains unchanged. Sparse island clusters and varied tower silhouettes break up repeated scenery. On Palm and Port, a three-dolphin pod breaches near the first third of the course when approached, then dives away, once per lap. Their bodies arch through each jump and flex as they swim away. Storm has a distant cargo boat cruising beyond the offshore ridge with navigation lights and a small wake. The menu and HUD are semantic HTML over the WebGL canvas.
...
Browser captures are in `artifacts/`. The showcase pass covers Chromium full laps and WebKit phone setup/checkpoint rotation. Physical phones, hardware gamepad feel, speaker/headphone balance and separate-network multiplayer still need hands-on verification. Before the First Credit loops on the title screen when sound is enabled. Starting a race switches to its fixed course song; returning to the title screen restores its theme. Sapphire Wake loops on Palm Circuit, Neon Slipway on Port Afterdark, and Chrome Horizon on Storm Signal. Start Race enables audio when starting a race, and SOUND toggles music plus engine effects, with a speaker icon on mobile. A quiet low motor rumble and water rush follow speed, throttle, water contact and boost. Collecting an item plays a rising cue; torpedoes, mines, boosts and wake emitters have distinct activation sounds. These cues follow confirmed inventory changes for solo and multiplayer racers. A cyan energy plume extends from the stern nozzle under throttle, grows with speed and boost, and fades when coasting or braking. Pause freezes playback. A dedicated music analyser extracts bass, mid, and treble energy with adaptive peaks and smooth decay: nearby course lights, sun scale, bloom, wave-crest highlights and reflections respond without changing collision geometry or wave physics. The ocean keeps a steady base color. A gently filtered music signal adds a small response to existing surface glints and breaking foam, while the hull wake picks up subtle color changes. Bass accents remain in the distant illuminated scenery. Music never changes handling or checkpoint timing. Pause and mute clear the surface accents and let the wake fade; reduced-motion preferences disable the additional visual modulation. Music files load on demand.
...
The approved promotional artwork is served at `public/og.png`. Its dark teal water, burgundy Palm sky and orange sun guide the in-game palette. Narrow highlights keep riders separate from the water. A broken foam trail widens behind each hull and the same displaced surface used by physics; fuller angular spray responds to carving and landing. Foam fades after takeoff without bridging the airborne path, clears after teleports and recovery, uses one draw call and has a fixed twelve-rider capacity. Landing adds a small camera compression without rolling the horizon, suppressed by reduced motion.
