...
it('six AI riders can finish a full race while collecting and using weapons', () => {
  const track = TRACKS[0],
    racers = Array.from({ length: 6 }, (_, i) => createRacer(track, i));
  let seed = 19,
    used = 0;
  ...
  expect(racers.every((r) => [r.x, r.y, r.z, r.vx, r.vy, r.vz].every(Number.isFinite))).toBe(true);
}, 30000);
